...
- Barracks, Farm, Medical Bay, Workshop, Watchtower → composite scenes in
  ...
  Kit format: the shipped GLTF conversions had BROKEN flat UVs (every
  wall sampled one gray palette texel) — the kit now uses the FBX
  originals (proper UVs) with the real `New Palitra.jpg` palette sitting
  BESIDE the models in each Models/ dir. NEVER reference a model's
  texture via `../` paths: editor/CI import resolves them (CI stays
  ...
  texture beside the model, always. Player-build wall/foundation/floor
  pieces use authored low-poly plank models instead
  (`assets/base_pieces/*.gltf`, generated by
  `tools/gen_base_piece_models.py`: vertex-colored planks/beams/nails,
  one material, exact kit panel frames; per-instance look via
  BuildingPiece.variation_tints — spot-hash picks a shared tint
  material). Pack
